...
{
    int r = libusb_control_transfer(ce_cam_get_cam_handle(cam_num),
                                    RT_H2D,
                                    0xA9,
                                    0xFF00,
                                    0,
...
// 双目设备 rom 读接口
static std::string ce_cam_i2c_readrom(int cam_num)
{
...
static void* ce_cam_showimg(lrImg &plr)
{
...
int ce_cam_showimg_init(lrImg &plr)
{
    ...
    //}
    std::thread mythread(ce_cam_showimg, std::ref(plr));
    mythread.detach();
    return SUCCESS;
}
...
{
    ce_cam_preprocess_stop_run = false;
...
libusb_device_handle *pcams1n0_handle;
libusb_device_handle *pcams1n1_handle;
libusb_device_handle *pcams1n2_handle;

pthread_t ce_cams1n0_capture_thread = 0;
pthread_t ce_cams1n1_capture_thread = 0;
pthread_t ce_cams1n2_capture_thread = 0;
bool ce_cams1_capture_stop_run=false;


pthread_t ce_cams1_showimg_thread = 0;
bool ce_cams1_showimg_stop_run = false;

pthread_t ce_cams1_preprocess_thread = 0;
bool ce_cams1_preprocess_stop_run = false;


threadsafe_queue<img_pkg *> img_pkg_s1n0_list;
threadsafe_queue<img_pkg *> img_pkg_s1n1_list;
threadsafe_queue<img_pkg *> img_pkg_s1n2_list;
...
static libusb_device_handle* ce_cams1_get_cam_handle(int cam_num)
{
    ...
    if(CAMS1_N0 == cam_num)
        return pcams1n0_handle;
    else if(CAMS1_N1 == cam_num)
        return pcams1n1_handle;
    else if(CAMS1_N2 == cam_num)
        return pcams1n2_handle;
    else
    {
        LOG("celog: Wrong cam number!\r\n");
        LOG("celog: cam_num = %d \r\n",cam_num);
...
static int ce_cams1_ctrl_camera(int cam_num, unsigned char instruction)
{
    unsigned char buf[1];
    int r = libusb_control_transfer(ce_cams1_get_cam_handle(cam_num),RT_H2D,instruction,0,0,buf,0,1000);
    if(r != 0)
    {
        LOG("celog: cam%d,i:0x%02X,r:%d\r\n",cam_num,instruction,r);
    }
...
static int ce_cams1_i2c_read(int cam_num, unsigned char reg,int* value)
{
    unsigned char buf[3];
    int r = libusb_control_transfer(ce_cams1_get_cam_handle(cam_num),RT_D2H,CAM_I2C_R,(CAM_I2C_ADDR<<8)+reg,0,buf,3,1000);
    if(r != 3)
    {
        LOG("celog: cam%d,i:0x%02X,r:%d\r\n",cam_num,CAM_I2C_R,r);
        return r;
...
static int ce_cams1_i2c_write(int cam_num, unsigned char reg,int value)
{
    unsigned char buf[1];
    int r = libusb_control_transfer(ce_cams1_get_cam_handle(cam_num),RT_D2H,CAM_I2C_W,(CAM_I2C_ADDR<<8)+reg,value,buf,1,1000);
    if(r != 1)
    {
        LOG("celog: cam%d,i:0x%02X,r:%d\r\n",cam_num,CAM_I2C_W,r);
        return r;
...
static std::string ce_cams1_i2c_readrom(int cam_num)
{
    unsigned char buf[64];
    int r = libusb_control_transfer(ce_cams1_get_cam_handle(cam_num),RT_D2H,0xA9,0xFF00,0,buf,64,1000);
    if(r != 64)
    {
        LOG("celog: cam%d,i:0x%02X,r:%d\r\n",cam_num,0xA9,r);
        return std::string("0");
...
static int ce_cams1_i2c_writerom(int cam_num, unsigned char* buf)
{
    //unsigned char buf[64];
    int r = libusb_control_transfer(ce_cams1_get_cam_handle(cam_num),RT_H2D,0xA9,0xFF00,0,buf,64,1000);
    if(r != 64)
    {
        LOG("celog: cam%d,i:0x%02X,r:%d\r\n",cam_num,0xA9,r);
        return r;
...
static void ce_cams1_set_mt9v034_config_default(int camlr)
{
    usleep(10000);

//    unsigned char buf[64] = {8,8,8,2,3,4,5,6,7,8,9};
//    ce_cams1_i2c_writerom(camlr, buf);
//    ce_cams1_i2c_readrom(camlr);
//    ce_cams1_i2c_write(camlr,0x07,0x0188);

    // CONTEXT A
    ce_cams1_i2c_write(camlr,0x04,0x02F0);    // 720X480
    ce_cams1_i2c_write(camlr,0x03,0x01E0);
    ce_cams1_i2c_write(camlr,0x05,0x0106);    // HB
    ce_cams1_i2c_write(camlr,0x06,0x024B);    // VB
    ce_cams1_i2c_write(camlr,0x0D,0x0300);
    ce_cams1_i2c_write(camlr,0x01,0x0001);
    ce_cams1_i2c_write(camlr,0x02,0x0004);
    ce_cams1_i2c_write(camlr,0x08,0x01BB);
    ce_cams1_i2c_write(camlr,0x09,0x01D9);
    ce_cams1_i2c_write(camlr,0x0A,0x0164);

    // CONTEXT B
    ce_cams1_i2c_write(camlr,0xCC,0x02F0);
    ce_cams1_i2c_write(camlr,0xCB,0x01E0);
    ce_cams1_i2c_write(camlr,0xCD,0x0106);
    ce_cams1_i2c_write(camlr,0xCE,0x024B);
    ce_cams1_i2c_write(camlr,0x0E,0x0300);
    ce_cams1_i2c_write(camlr,0xC9,0x0001);
    ce_cams1_i2c_write(camlr,0xCA,0x0004);
    ce_cams1_i2c_write(camlr,0xCF,0x01BB);
    ce_cams1_i2c_write(camlr,0xD0,0x01D9);
    ce_cams1_i2c_write(camlr,0xD1,0x0164);



    ce_cams1_i2c_write(camlr,0x70,0x0303);

    ce_cams1_i2c_write(camlr,0x0F,0x0000);
    ce_cams1_i2c_write(camlr,0xAC,0x0001);
    ce_cams1_i2c_write(camlr,0xAD,0x01E0);
    ce_cams1_i2c_write(camlr,0xAB,0x0040);
    ce_cams1_i2c_write(camlr,0xB0,0xFFFF);
    ce_cams1_i2c_write(camlr,0xA5,0x0037);
    ce_cams1_i2c_write(camlr,0x1C,0x0202);
    ce_cams1_i2c_write(camlr,0x7F,0x0000);
    ce_cams1_i2c_write(camlr,0xA6,0x0001);
    ce_cams1_i2c_write(camlr,0xA8,0x0000);
    ce_cams1_i2c_write(camlr,0xA9,0x0000);
    ce_cams1_i2c_write(camlr,0xAA,0x0000);

    ce_cams1_i2c_write(camlr,0x34,0x0003);
    ce_cams1_i2c_write(camlr,0x3c,0x0003);
    ce_cams1_i2c_write(camlr,0x2c,0x0004);


    ce_cams1_i2c_write(camlr,0x25,0x0020);
    ce_cams1_i2c_write(camlr,0xC2,0x0840);
    ce_cams1_i2c_write(camlr,0x0C,0x0001);
    ce_cams1_i2c_write(camlr,0x47,0x0080);
    ce_cams1_i2c_write(camlr,0x48,0x0000);
    ce_cams1_i2c_write(camlr,0x4C,0x0002);
    ce_cams1_i2c_write(camlr,0x35,0x0028);
    ce_cams1_i2c_write(camlr,0x36,0x0028);
    ce_cams1_i2c_write(camlr,0xA5,0x0030);
    ce_cams1_i2c_write(camlr,0xAD,0x01E0);
    ce_cams1_i2c_write(camlr,0xAB,0x0040);
...
static void ce_cams1_set_mt9v034_fps(int camlr)
{
    ce_cams1_i2c_write(camlr,0x05,ce_config_get_cf_img_HB());
    ce_cams1_i2c_write(camlr,0x06,ce_config_get_cf_img_VB());
}

static void ce_cams1_set_mt9v034_EG_mode(int camlr)
{
   switch (ce_config_get_cf_cam_EG_mode()){
    case 0:
        ce_cams1_i2c_write(camlr,0xAF,0x00);      //AEC
        ce_cams1_i2c_write(camlr,0x0B,ce_config_get_cf_cam_man_exp());   //Exposure Time
        ce_cams1_i2c_write(camlr,0x35,ce_config_get_cf_cam_man_gain());  //Gain
        break;
    case 1:
        ce_cams1_i2c_write(camlr,0xA5,ce_config_get_cf_cam_auto_EG_des());
        ce_cams1_i2c_write(camlr,0xA6,0x01);
        ce_cams1_i2c_write(camlr,0xAC,ce_config_get_cf_cam_auto_EG_bottom());
        ce_cams1_i2c_write(camlr,0xAD,ce_config_get_cf_cam_auto_EG_top());
        ce_cams1_i2c_write(camlr,0xAE,2);
        break;
    case 2:
        ce_cams1_i2c_write(camlr,0xAF,0x01);
        ce_cams1_i2c_write(camlr,0xA5,ce_config_get_cf_cam_auto_EG_des());
        ce_cams1_i2c_write(camlr,0xA6,0x01);
        ce_cams1_i2c_write(camlr,0xA8,0x00);
        ce_cams1_i2c_write(camlr,0xAC,ce_config_get_cf_cam_auto_E_man_G_Ebottom());
        ce_cams1_i2c_write(camlr,0xAD,ce_config_get_cf_cam_auto_E_man_G_Etop());
        ce_cams1_i2c_write(camlr,0xAE,2);
        ce_cams1_i2c_write(camlr,0x35,ce_config_get_cf_cam_auto_E_man_G());
        break;
    case 3:

        break;
    case 4:
        ce_cams1_i2c_write(camlr,0xA6,ce_config_get_cf_cam_agc_aec_skip_frame());
        ce_cams1_i2c_write(camlr,0xA8,2);
        ce_cams1_i2c_write(camlr,0xA9,ce_config_get_cf_cam_agc_aec_skip_frame());
        ce_cams1_i2c_write(camlr,0xAA,2);
        break;
    default:
        break;
    }
...
static void *ce_cams1_capture(void *pUserPara)
{
    ...
    ce_cams1_set_mt9v034_config_default(camlr);
    ce_cams1_set_mt9v034_fps(camlr);
    ce_cams1_set_mt9v034_EG_mode(camlr);
    ce_cams1_ctrl_camera(camlr,SET_MCLK_48MHz);

    usleep(1000);

    ce_cams1_i2c_write(camlr,0x0C,0x0001);
    ce_cams1_ctrl_camera(camlr,STANDBY_SHORT);
    ...
    if(CAMS1_N0 == camlr)
    {
        tlist = &img_pkg_s1n0_list;

    }
    else if(CAMS1_N1 == camlr)
    {
        tlist = &img_pkg_s1n1_list;
    }
    else if(CAMS1_N2 == camlr)
    {
        tlist = &img_pkg_s1n2_list;
    }

    libusb_device_handle *pcam_handle;
    pcam_handle = ce_cams1_get_cam_handle(camlr);

    while(!ce_cams1_capture_stop_run)
    {
        timg_pkg = new img_pkg;
        if (NULL == timg_pkg)
        {
    ...
    if(CAMS1_N0 == camlr)
    {
        ce_cams1n0_capture_thread = 0;
    }
    else if(CAMS1_N1 == camlr)
    {
        ce_cams1n1_capture_thread = 0;
    }
    else if(CAMS1_N2 == camlr)
    {
        ce_cams1n2_capture_thread = 0;
    }
...
static void ce_cams1_get_soft_version(int camlr)
{
    ...
    if(CAMS1_N0 == camlr)
    {
        LOG("celog: the s1n0 cam version :");
    }
    else if(CAMS1_N1 == camlr)
    {
        LOG("celog: the s1n1 cam version :");
    }
    else if(CAMS1_N2 == camlr)
    {
        LOG("celog: the s1n2 cam version :");
    }
...
{
    ce_cams1_capture_stop_run=true;
    usleep(100);
    if(ce_cams1n0_capture_thread !=0)
    {
        pthread_join(ce_cams1n0_capture_thread,NULL);
    }
    if(ce_cams1n1_capture_thread !=0)
    {
        pthread_join(ce_cams1n1_capture_thread,NULL);
    }
    if(ce_cams1n2_capture_thread !=0)
    {
        pthread_join(ce_cams1n2_capture_thread,NULL);
    }
    ce_usb_close();
}
...
{
    static int i=0;
    cv::Mat img_pf(cv::Size(ce_config_get_cf_img_width(),ce_config_get_cf_img_height()),CV_8UC1);
    img_pkg *img_pkg_pf = nullptr;
    int iter = 0;
    while(1){
        if(!img_pkg_s1n0_list.empty()){
            img_pkg_s1n0_list.try_front(img_pkg_pf);
            img_pkg_s1n0_list.try_pop(img_pkg_pf);
            memcpy(img_pf.data, img_pkg_pf, ce_config_get_cf_img_size());
        }
        delete img_pkg_pf;
        img_pkg_pf = NULL;
        usleep(1000);
...
static void* pf_ce_cams1_preprocess(cv::Mat &ref_img)
{
    int i=0;
    cv::Mat img_pf(cv::Size(ce_config_get_cf_img_width(),ce_config_get_cf_img_height()),CV_8UC1);
    img_pkg *img_pkg_pf = nullptr;
...
int pf_ce_cams1_preprocess_init(cv::Mat &ref_img){
    ce_cams1_preprocess_stop_run = false;
    //int temp = pthread_create(&ce_cams1_preprocess_thread, NULL, pf_ce_cams1_preprocess, std::ref(ref_img));
    std::thread trd(pf_ce_cams1_preprocess, std::ref(ref_img));
    trd.detach();
    return SUCCESS;
}
...
int ce_cams1_capture_init()
{
    ...
    for(int i=0; i<r; i++)
    {
        ...
        else
        {
            if(CAMS1_N0 == buf)
            {
                pcams1n0_handle = pusb_handle;
            }
            else if(CAMS1_N1 == buf)
            {
                pcams1n1_handle = pusb_handle;
            }
            else if(CAMS1_N2 == buf)
            {
                pcams1n2_handle = pusb_handle;
            }
        }
    }
    ...
    {
        int temp=0;
        static int cams1n0_addr = CAMS1_N0;
        static int cams1n1_addr = CAMS1_N1;
        static int cams1n2_addr = CAMS1_N2;

        ce_cams1_capture_stop_run = false;
        if(pcams1n0_handle != NULL)
        {
            ce_cams1_get_soft_version(cams1n0_addr);

            temp = pthread_create(&ce_cams1n0_capture_thread, NULL, ce_cams1_capture, &cams1n0_addr);
            if(temp)
            {
                LOG("celog: create thread failed !\r\n");
                return CE_ERROR;
        ...
        if(pcams1n1_handle != NULL)
        {
            ce_cams1_get_soft_version(cams1n1_addr);

            temp = pthread_create(&ce_cams1n1_capture_thread, NULL, ce_cams1_capture, &cams1n1_addr);
            if(temp)
            {
                LOG("celog: create thread failed !\r\n");
                return CE_ERROR;
        ...
        if(pcams1n2_handle != NULL)
        {
            ce_cams1_get_soft_version(cams1n2_addr);

            temp = pthread_create(&ce_cams1n2_capture_thread, NULL, ce_cams1_capture, &cams1n2_addr);
            if(temp)
            {
                LOG("celog: create thread failed !\r\n");
                return CE_ERROR;
...
{
    pcaml_handle = caml_handle_;
    pcamr_handle = camr_handle_;
    pcams1n0_handle = cam0_handle_;
    pcams1n1_handle = cam1_handle_;
    pcams1n2_handle = cam2_handle_;
}

// 获得双目设备的 id
std::string ce_cam_get_id(int cam_num)
{
    std::string ret;
    ret = ce_cam_i2c_readrom(cam_num);

    return ret;
}

// 获得单目设备的 id
std::string ce_cams1_get_id(int cam_num)
{
    std::string ret;
    ret = ce_cams1_i2c_readrom(cam_num);

    return ret;
}

// 设置双目设备的 id
int ce_cam_set_id(int cam_num, int serial)
{
    int ret = 0;
    unsigned char buf[64];
    ...
    ret = ce_cam_i2c_writerom(cam_num, buf);

    return ret;
}

// 设置单目设备的 id
int ce_cams1_set_id(int cam_num, int serial)
{
    int ret = 0;
    unsigned char buf[64];
    memset(buf, 0, sizeof(unsigned char) * 64);
    ...
    ret = ce_cams1_i2c_writerom(cam_num, buf);

    return ret;
}


int set_ce_cam_id(int num, int serial)
{
    int ret = 0;
    unsigned char buf[64];
    memset(buf, 0, sizeof(unsigned char) * 64);
    ...
    ret = ce_cams1_i2c_writerom(num, buf);
}
